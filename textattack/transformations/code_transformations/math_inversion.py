...
class MathInversionVisitor(OneByOneVisitor):
    ...
    def is_transformable(self, node):
        return isinstance(node, ast.BinOp) and (isinstance(node.op, ast.Add) or
    ...
    def transform_node(self, node):
        if isinstance(node.op, ast.Add):
            ...
            node.op = ast.Mult()
            node.right = ast.BinOp(op=ast.Mult(), left=ast.Constant(1.0), right=node.right)
            return ast.Call(
                ...
                args=[
                    ast.BinOp(
                        left=node.left,
                        op=ast.Div(),
                        right=ast.BinOp(left=ast.Constant(1.0), op=ast.Div(), right=node.right)
                    )
                ],
                keywords=[]
            )
